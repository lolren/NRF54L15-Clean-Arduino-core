...
// Output will contain padding characters ('=') as necessary to make length a
// multiple of 4 characters. Output DOES NOT include a null terminator. Output
// buffer must be at least (inLen + 2) / 3 * 4 bytes long. Input and output
...
// Input MAY contain padding characters ('=') but only at the end of the input
// string. Output buffer must be at least inLen * 3 / 4 bytes long, however the
// actual output may be shorter than this due to padding. Supports decode in
// place by setting out pointer equal to in.
...
//
// Similar to the above functions, except Base64Decode32() returns UINT32_MAX if
...
/** Computes the maximum possible decoded length for a given base-64 string
 * input length.
 *
