...
// Convert a base64 character to a value in the range 0..63, or UINT8_MAX if
...
// Convert a value in the range 0..63 to its equivalent base64url character
// (see RFC-4648, section 5). Return '=' for any value >= 64.
...
// Convert a base64url character to a value in the range 0..63, or UINT8_MAX if
// the character is invalid.
...
uint32_t Base64Encode32(const uint8_t * in, uint32_t inLen, char * out, Base64ValToCharFunct valToCharFunct)
{
    ...
    // Maximum number of input bytes to convert to base-64 in a single call to
    // Base64Encode. Number is the largest multiple of 3 bytes where the
    // resulting number of base-64 characters fits within a uint16_t.
    enum
    {
...
uint16_t Base64Decode(const char * in, uint16_t inLen, uint8_t * out, Base64CharToValFunct charToValFunct)
{
    ...
    while (inLen > 0 && isgraph(*in))
    {
        ...
        *out++ = static_cast<uint8_t>((a << 2) | (b >> 4));
        ...
        a = charToValFunct(static_cast<uint8_t>(*in++));
        inLen--;
        if (a == UINT8_MAX)
            goto fail;

        *out++ = static_cast<uint8_t>((b << 4) | (a >> 2));

        if (inLen == 0 || *in == '=')
            break;

        b = charToValFunct(static_cast<uint8_t>(*in++));
        inLen--;
        if (b == UINT8_MAX)
            goto fail;

        *out++ = static_cast<uint8_t>((a << 6) | b);
    }
...
uint32_t Base64Decode32(const char * in, uint32_t inLen, uint8_t * out, Base64CharToValFunct charToValFunct)
{
    ...
    // Maximum number of input bytes to convert from base-64 in a single call
    // to Base64Decode. Number is the largest multiple of 4 bytes where the
    // resulting number of raw bytes fits within a uint16_t.
    enum
    {
        kMaxConvert = (UINT16_MAX / 3) * 4
    };
    ...
    while (true)
    {
        ...
        if (inLen > kMaxConvert && (inChunkLen % 4) != 0)
        {
            inChunkLen = static_cast<uint16_t>(inChunkLen - (inChunkLen % 4));
        }
