...
constexpr uint8_t kMaxAssociationCandidateAttempts = 4U;
...
bool candidateIsExcluded(uint8_t channel, const ZigbeeMacBeaconView& beacon,
                         const ZigbeeBeaconCandidate* excludedCandidates,
                         uint8_t excludedCount) {
  if (!beacon.valid || excludedCandidates == nullptr || excludedCount == 0U) {
    return false;
  }
  for (uint8_t i = 0U; i < excludedCount; ++i) {
    const ZigbeeBeaconCandidate& excluded = excludedCandidates[i];
    if (!excluded.valid) {
      continue;
    }
    if (excluded.channel != channel || excluded.beacon.panId != beacon.panId ||
        excluded.beacon.sourceShort != beacon.sourceShort ||
        excluded.beacon.network.extendedPanId !=
            beacon.network.extendedPanId) {
      continue;
    }
    return true;
  }
...
bool activeScanForMask(ZigbeeRadio& radio, uint8_t* ioMacSequence,
  ...
  const uint32_t requestRetryIntervalMs =
      (scanWindowMs >= 240UL) ? 80UL : ((scanWindowMs >= 160UL) ? 60UL : 45UL);
  bool found = false;
  for (uint8_t i = 0U; i < channelCount; ++i) {
    ...
    uint32_t nextRequestAt = millis();
    while (static_cast<int32_t>(millis() - deadline) < 0) {
      ZigbeeFrame frame{};
      const bool issueRequest =
          static_cast<int32_t>(millis() - nextRequestAt) >= 0;
      const bool received = issueRequest
                                ? radio.transmitThenReceive(
                                      request, requestLength, &frame, 60000U,
                                      false, 1200000UL)
                                : radio.receive(&frame, 5000U, 300000UL);
      if (issueRequest) {
        nextRequestAt += requestRetryIntervalMs;
      }
      if (!received) {
        continue;
      }

      Serial.print("scan_rx ch=");
      Serial.print(channel);
      Serial.print(" len=");
      Serial.print(frame.length);
      Serial.print(" rssi=");
...
#endif
      if (candidateIsExcluded(channel, beacon, excludedCandidates,
                              excludedCount)) {
#if NRF54L15_CLEAN_ZIGBEE_SCAN_TRACE
        traceBeaconCandidate(channel, frame.rssiDbm, beacon, "excluded_parent",
                             score);
#endif
        continue;
      }
  ...
  Serial.print("scan_done found=");
  Serial.print(found ? "yes" : "no");
  Serial.print("\r\n");
  return found;
}
...
bool waitForAssociationResponse(const ZigbeeCommissioningPolicy& policy,
  ...
  const uint32_t listenWindowMs =
      (policy.associationPollListenMs != 0UL) ? policy.associationPollListenMs
                                              : 120UL;
  const uint32_t listenWindowUs = listenWindowMs * 1000UL;
  ...
  while (static_cast<int32_t>(millis() - deadline) < 0) {
    Serial.print("assoc_poll parent=0x");
    Serial.print(parentShort, HEX);
    Serial.print(" pan=0x");
    ...
    const uint32_t listenWindowStartUs = micros();
    bool firstWindowReceive = true;
    while (static_cast<uint32_t>(micros() - listenWindowStartUs) <
           listenWindowUs) {
      const uint32_t elapsedUs =
          static_cast<uint32_t>(micros() - listenWindowStartUs);
      ...
      const bool received =
          firstWindowReceive
              ? radio.transmitThenReceive(pollFrame, pollLength, &frame,
                                          remainingUs, false, 1200000UL)
              : radio.receive(&frame, remainingUs, 350000UL);
      if (firstWindowReceive) {
        const ZigbeeTransmitDebug pollDebug = radio.lastTransmitDebug();
        Serial.print("assoc_poll_dbg end=");
        Serial.print(pollDebug.endSeen ? "yes" : "no");
        Serial.print(" ack_req=");
        Serial.print(pollDebug.ackRequested ? "yes" : "no");
        Serial.print(" ack_rx=");
        Serial.print(pollDebug.ackReceived ? "yes" : "no");
        Serial.print(" rx=");
        Serial.print(pollDebug.rxDone ? "yes" : "no");
        Serial.print(" crc=");
        Serial.print(pollDebug.crcOk ? "yes" : "no");
        Serial.print(" len=");
        Serial.print(pollDebug.rxLength);
        Serial.print(" tx=");
        Serial.print((pollDebug.endSeen && pollDebug.disabledSeen) ? "yes"
                                                                   : "no");
        Serial.print("\r\n");
        firstWindowReceive = false;
      }
      if (!received) {
        continue;
      }

      Serial.print("assoc_rx len=");
      Serial.print(frame.length);
      Serial.print(" rssi=");
      Serial.print(frame.rssiDbm);
      Serial.print("\r\n");
      ...
      *outAssignedShort = response.assignedShort;
      Serial.print("assoc_ok short=0x");
      Serial.print(response.assignedShort, HEX);
      Serial.print("\r\n");
      return true;
    }
...
bool attemptAssociationRequest(ZigbeeRadio& radio, uint8_t* ioMacSequence,
  ...
  for (uint8_t attempt = 0U; attempt < 4U; ++attempt) {
    ...
    if (radio.transmit(request, requestLength, false, 1200000UL)) {
      transmitted = true;
      if (attempt != 0U) {
        Serial.print("assoc_req_retry_ok attempt=");
    ...
    const ZigbeeTransmitDebug debug = radio.lastTransmitDebug();
    Serial.print("assoc_req_retry attempt=");
    Serial.print(attempt + 1U);
    Serial.print(" end=");
  ...
  if (!transmitted) {
    Serial.print("assoc_req_fail parent=0x");
    Serial.print(parentShort, HEX);
    Serial.print(" pan=0x");
    Serial.print(panId, HEX);
    Serial.print("\r\n");
  ...
  Serial.print("assoc_req_ok parent=0x");
  Serial.print(parentShort, HEX);
  Serial.print(" pan=0x");
  Serial.print(panId, HEX);
  Serial.print("\r\n");
  ...
  if (!waitForAssociationResponse(state->policy, radio, ioMacSequence, panId,
    ...
    Serial.print("assoc_timeout parent=0x");
    Serial.print(parentShort, HEX);
    Serial.print(" pan=0x");
    Serial.print(panId, HEX);
    Serial.print("\r\n");
...
bool validTrustCenterSource(const ZigbeeEndDeviceCommonState& state,
  ...
  if (apsSecured) {
    ...
    // Trust Center APS-secured commissioning commands can legitimately arrive
    // either via the associated parent or directly from the coordinator. Some
    // Zigbee2MQTT / HA topologies with router parents deliver Transport Key /
    // Update Device from coordinator short 0x0000 even when parentShort is a
    // router.
    if (state.coordinatorShort != 0U && sourceShort == state.coordinatorShort) {
      return true;
    }
    if (state.parentShort != 0U && sourceShort == state.parentShort) {
      return true;
    }
    return (state.parentShort == 0U && state.coordinatorShort == 0U &&
            sourceShort == 0U);
  }
...
bool scanForKnownNetwork(ZigbeeRadio& radio, uint8_t* ioMacSequence,
  ...
  for (uint8_t maskIndex = 0U; maskIndex < maskCount; ++maskIndex) {
    if (!scanKnownNetworkForMask(radio, ioMacSequence, scanMasks[maskIndex],
                                 *state, excludedCandidates, excludedCount,
                                 outResult)) {
      continue;
    }
...
bool attemptAssociationViaCandidates(
    ZigbeeRadio& radio, uint8_t* ioMacSequence, uint64_t localIeee,
    uint8_t capabilityInformation, ZigbeeEndDeviceCommonState* state,
    bool knownNetworkOnly, uint16_t* outAssignedShort) {
  if (ioMacSequence == nullptr || state == nullptr || outAssignedShort == nullptr) {
    return false;
  }

  ZigbeeBeaconCandidate attempted[kMaxAssociationCandidateAttempts] = {};
  uint8_t attemptedCount = 0U;

  while (attemptedCount < kMaxAssociationCandidateAttempts) {
    ZigbeeBeaconCandidate candidate{};
    const bool found =
        knownNetworkOnly
            ? scanForKnownNetwork(radio, ioMacSequence, state, &candidate,
                                  attempted, attemptedCount)
            : ZigbeeCommissioning::activeScan(radio, ioMacSequence, state,
                                              &candidate, attempted,
                                              attemptedCount);
    ...
    if (attemptAssociationRequest(radio, ioMacSequence, state->panId,
                                  ...
                                  &assignedShort)) {
      *outAssignedShort = assignedShort;
      return true;
    }

    attempted[attemptedCount++] = candidate;
    if (state->lastFailure != ZigbeeCommissioningFailure::kAssociationTimeout &&
        state->lastFailure !=
            ZigbeeCommissioningFailure::kAssociationRequestFailed) {
      return false;
    }
  }
...
bool ZigbeeCommissioning::scoreBeacon(const ZigbeeCommissioningPolicy& policy,
  ...
  score += beacon.panCoordinator ? 96 : 0;
...
void ZigbeeCommissioning::initializeEndDeviceState(
    ...
    uint16_t coordinatorShort, ZigbeeLogicalType logicalType) {
  if (state == nullptr) {
    return;
  }
...
void ZigbeeCommissioning::restoreEndDeviceState(
  ...
  initializeEndDeviceState(state, policy, preferredChannel, preferredPanId,
                           defaultShort, coordinatorShort, state->logicalType);

  if (persistent.ieeeAddress != localIeee) {
    return;
  }

  if (persistent.logicalType <=
      static_cast<uint8_t>(ZigbeeLogicalType::kEndDevice)) {
    state->logicalType =
        static_cast<ZigbeeLogicalType>(persistent.logicalType);
  }
...
void ZigbeeCommissioning::clearEndDeviceState(
  ...
  const uint16_t defaultShort = state->defaultShort;
  const uint16_t coordinatorShort = state->coordinatorShort;
  const ZigbeeLogicalType logicalType = state->logicalType;
  ...
  initializeEndDeviceState(state, policy, preferredChannel, preferredPanId,
                           defaultShort, coordinatorShort, logicalType);
...
bool ZigbeeCommissioning::shouldAttemptSecureRejoin(
    const ZigbeeEndDeviceCommonState& state) {
  // NWK secure rejoin authenticates with the active network key. A device that
  // joined via the well-known TC link key still has enough context to attempt
  // a standards-compliant secure rejoin after wake or reset, as long as the
  // network key and Trust Center identity were retained.
...
void ZigbeeCommissioning::requestNetworkSteering(
  ...
  // Fresh network steering must not carry over an old network-key context.
  // Otherwise the joiner can enter waiting_transport_key while still behaving
  // like a secured member and silently drop the legitimate unsecured NWK
  // Transport Key from the Trust Center.
  clearNetworkKey(state);
  clearAlternateNetworkKey(state);
  state->incomingNwkFrameCounter = 0U;
  state->incomingApsFrameCounter = 0U;
  state->securityEnabled = false;
  state->trustCenterIeee = state->policy.pinnedTrustCenterIeee;
  state->preconfiguredKeyMode = ZigbeePreconfiguredKeyMode::kNone;
  state->deviceAnnouncePending = false;
  state->endDeviceTimeoutPending = false;
  state->endDeviceTimeoutNegotiated = false;
  state->lastDeviceAnnounceMs = 0U;
...
bool ZigbeeCommissioning::usesParentPolling(
    const ZigbeeEndDeviceCommonState& state) {
...
bool ZigbeeCommissioning::shouldRequestEndDeviceTimeout(
    const ZigbeeEndDeviceCommonState& state) {
  return usesParentPolling(state) && state.joined && state.securityEnabled &&
         state.haveActiveNetworkKey && state.endDeviceTimeoutPending;
}
...
ZigbeeCommissioningAction ZigbeeCommissioning::nextAction(
  ...
  if (state->state == ZigbeeCommissioningState::kWaitingTransportKey) {
    if (elapsedSinceAttempt < state->policy.transportKeyTimeoutMs) {
      return usesParentPolling(*state) ? ZigbeeCommissioningAction::kPollParent
...
bool ZigbeeCommissioning::activeScan(ZigbeeRadio& radio,
  ...
  for (uint8_t maskIndex = 0U; maskIndex < maskCount; ++maskIndex) {
    if (!activeScanForMask(radio, ioMacSequence, scanMasks[maskIndex],
                           state->policy, excludedCandidates, excludedCount,
                           outResult)) {
      continue;
    }
    found = outResult->valid;
...
bool ZigbeeCommissioning::performJoin(ZigbeeRadio& radio,
  ...
  ++state->joinAttempts;
  state->lastJoinAttemptMs = millis();
  Serial.print("join_attempt #");
  Serial.print(state->joinAttempts);
  Serial.print("\r\n");

  uint16_t assignedShort = 0U;
  if (!attemptAssociationViaCandidates(radio, ioMacSequence, localIeee,
                                       capabilityInformation, state, false,
                                       &assignedShort)) {
    Serial.print("join_assoc_fail reason=");
    Serial.print(failureName(state->lastFailure));
    Serial.print("\r\n");
    return false;
  }
...
bool ZigbeeCommissioning::performSecureRejoin(
  ...
  if (!associated) {
    associated = attemptAssociationViaCandidates(
        radio, ioMacSequence, localIeee, capabilityInformation, state, true,
        &assignedShort);
    if (!associated) {
      return false;
    }
...
bool ZigbeeCommissioning::acceptTransportKeyCommand(
  ...
  if (!transportKeyParsed) {
    Serial.print("transport_key_reject reason=parse_fail state=");
    Serial.print(stateName(state.state));
    Serial.print(" len=");
    Serial.print(length);
    Serial.print(" src=0x");
    Serial.print(sourceShort, HEX);
    Serial.print(" nwk_sec=");
    Serial.print(nwkSecured ? "1" : "0");
    Serial.print(" secured_ieee=0x");
    Serial.print(static_cast<uint32_t>(securedSourceIeee >> 32U), HEX);
    Serial.print(static_cast<uint32_t>(securedSourceIeee & 0xFFFFFFFFUL), HEX);
    Serial.print(" frame=");
    printHexBytes(frame, length);
    Serial.print("\r\n");
    return false;
  }
  ...
  if (!transportKey.valid) {
    Serial.print("transport_key_reject reason=invalid_command\r\n");
    return false;
  }
  if (transportKey.keyType != kZigbeeApsTransportKeyStandardNetworkKey) {
    Serial.print("transport_key_reject reason=key_type value=0x");
    Serial.print(transportKey.keyType, HEX);
    Serial.print("\r\n");
    return false;
  }
  if (transportKey.destinationIeee != localIeee) {
    Serial.print("transport_key_reject reason=dest_ieee cmd=0x");
    Serial.print(static_cast<uint32_t>(transportKey.destinationIeee >> 32U), HEX);
    Serial.print(static_cast<uint32_t>(transportKey.destinationIeee & 0xFFFFFFFFUL), HEX);
    Serial.print(" local=0x");
    Serial.print(static_cast<uint32_t>(localIeee >> 32U), HEX);
    Serial.print(static_cast<uint32_t>(localIeee & 0xFFFFFFFFUL), HEX);
    Serial.print("\r\n");
    return false;
  }
  if (expectedTc != 0U && transportKey.sourceIeee != expectedTc) {
    Serial.print("transport_key_reject reason=source_ieee cmd=0x");
    Serial.print(static_cast<uint32_t>(transportKey.sourceIeee >> 32U), HEX);
    Serial.print(static_cast<uint32_t>(transportKey.sourceIeee & 0xFFFFFFFFUL), HEX);
    Serial.print(" expected=0x");
    Serial.print(static_cast<uint32_t>(expectedTc >> 32U), HEX);
    Serial.print(static_cast<uint32_t>(expectedTc & 0xFFFFFFFFUL), HEX);
    Serial.print("\r\n");
    return false;
  }
  if (!validTrustCenterSource(state, sourceShort, effectiveSourceIeee,
                              effectiveSecured, apsSecurity.valid)) {
    Serial.print("transport_key_reject reason=trust_center_source src=0x");
    Serial.print(sourceShort, HEX);
    Serial.print(" parent=0x");
    Serial.print(state.parentShort, HEX);
    Serial.print(" coord=0x");
    Serial.print(state.coordinatorShort, HEX);
    Serial.print(" eff_ieee=0x");
    Serial.print(static_cast<uint32_t>(effectiveSourceIeee >> 32U), HEX);
    Serial.print(static_cast<uint32_t>(effectiveSourceIeee & 0xFFFFFFFFUL), HEX);
    Serial.print(" nwk_sec=");
    Serial.print(nwkSecured ? "1" : "0");
    Serial.print(" aps_sec=");
    Serial.print(apsSecurity.valid ? "1" : "0");
    Serial.print("\r\n");
    return false;
  }
  if (!apsSecurity.valid && state.policy.requireEncryptedTransportKey) {
    Serial.print("transport_key_reject reason=require_aps_security\r\n");
    return false;
  }
  if (apsSecurity.valid && apsSecurity.sourceIeee != transportKey.sourceIeee) {
    Serial.print("transport_key_reject reason=aps_source_ieee aps=0x");
    Serial.print(static_cast<uint32_t>(apsSecurity.sourceIeee >> 32U), HEX);
    Serial.print(static_cast<uint32_t>(apsSecurity.sourceIeee & 0xFFFFFFFFUL), HEX);
    Serial.print(" cmd=0x");
    Serial.print(static_cast<uint32_t>(transportKey.sourceIeee >> 32U), HEX);
    Serial.print(static_cast<uint32_t>(transportKey.sourceIeee & 0xFFFFFFFFUL), HEX);
    Serial.print("\r\n");
  ...
  // The first secured APS trust-center command of a fresh join may legally
  // start at frame counter 0. Only enforce monotonic replay checks once an
  // active network key has already been installed.
  if (apsSecurity.valid && state.haveActiveNetworkKey &&
      apsSecurity.frameCounter <= state.incomingApsFrameCounter) {
    Serial.print("transport_key_reject reason=aps_frame_counter fc=");
    Serial.print(apsSecurity.frameCounter);
    Serial.print(" last=");
    Serial.print(state.incomingApsFrameCounter);
    Serial.print("\r\n");
    return false;
  }
  if (!apsSecurity.valid && effectiveSourceIeee != 0U &&
      effectiveSourceIeee != transportKey.sourceIeee) {
    Serial.print("transport_key_reject reason=plaintext_source_ieee eff=0x");
    Serial.print(static_cast<uint32_t>(effectiveSourceIeee >> 32U), HEX);
    Serial.print(static_cast<uint32_t>(effectiveSourceIeee & 0xFFFFFFFFUL), HEX);
    Serial.print(" cmd=0x");
    Serial.print(static_cast<uint32_t>(transportKey.sourceIeee >> 32U), HEX);
  ...
  if (state.policy.installCodeOnly &&
      keyMode != ZigbeePreconfiguredKeyMode::kInstallCodeDerived) {
    Serial.print("transport_key_reject reason=install_code_only mode=");
    Serial.print(keyModeName(keyMode));
    Serial.print("\r\n");
    return false;
  }
  ...
  if (!state.haveActiveNetworkKey) {
    ...
    if (memcmp(transportKey.key, state.activeNetworkKey,
      ...
      Serial.print("transport_key_reject reason=active_key_mismatch seq=");
      Serial.print(transportKey.keySequence);
      Serial.print("\r\n");
      return false;
    }
    ...
    if (memcmp(transportKey.key, state.alternateNetworkKey,
      ...
      Serial.print("transport_key_reject reason=alternate_key_mismatch seq=");
      Serial.print(transportKey.keySequence);
      Serial.print("\r\n");
      return false;
    }
    ...
    Serial.print("transport_key_reject reason=stale_sequence seq=");
    Serial.print(transportKey.keySequence);
    Serial.print(" active=");
    Serial.print(state.activeNetworkKeySequence);
    Serial.print(" alternate=");
    Serial.print(state.alternateNetworkKeySequence);
    Serial.print("\r\n");
    return false;
  } else {
