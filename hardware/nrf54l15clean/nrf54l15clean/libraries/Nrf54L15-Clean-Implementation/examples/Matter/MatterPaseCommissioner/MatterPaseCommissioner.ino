// Matter PASE SPAKE2+ Commissioning — Commissioner
// =================================================
// Forms Thread network, derives verifier from PIN (20202021),
// waits for commissionee PBKDF request, runs SPAKE2+ handshake.
// Requires second board as commissionee. ~45s total.
// Output: serial 115200 baud, "pase DONE" = success.
...
enum PaseMsg : uint8_t { kPbkdfReq = 1, kPbkdfResp = 2, kSpake2p1 = 3, kSpake2p2 = 4, kSpake2p3 = 5 };
...
void onUdp(void*, const uint8_t* data, uint16_t len, const otMessageInfo& info) {
  ...
  Serial.print("pase rx type="); Serial.println(type); Serial.flush();
  ...
  if (ROLE == DemoRole::COMMISSIONEE && type == kSpake2p2) {
    ...
    // Compute shared secret similarly
    uint8_t X[65] = {0};
    computeSpake2pCommit(true, g_w0, X); // recompute X
    uint8_t concat[162] = {0};
    memcpy(concat, X, 65);
...
void startPase() {
  if (!g_thread.udpOpened()) return;  // Wait for UDP socket to be ready
  otIp6Address leader;
  if (!g_thread.getLeaderRloc(&leader)) return;
  
  // Generate random salt and send PBKDF param request with salt
  for (int i = 0; i < 32; i++) g_salt[i] = (uint8_t)(micros() ^ (millis() >> (i%8)));
  uint8_t req[33] = {kPbkdfReq};
  memcpy(req + 1, g_salt, 32);
  g_thread.sendUdp(leader, kPort, req, 33);
  Serial.println("pase pbkdf-req sent (with salt)"); Serial.flush();
}
...
void loop() {
  ...
  // Commissionee: start PASE when attached (retry every 5s until keys ready)
  static uint32_t lastPaseAttempt = 0;
  if (ROLE == DemoRole::COMMISSIONEE && g_thread.attached() && 
