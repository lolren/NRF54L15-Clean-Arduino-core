...
#error "Enable Tools > Zigbee Support to build ZigbeeHaDimmableLightStatic."
#endif
...
static constexpr uint16_t kLocalShort = 0x3345U;
...
static constexpr uint64_t kIeeeAddress = 0x00124B0001ABCE01ULL;
...
static void applyDefaultReporting() {
...
static void restoreState() {
  ...
  if (state.levelState > 0U || state.onOffState) {
    g_device.setLevel(state.levelState == 0U ? 0x80U : state.levelState);
  }
...
static void handleSerialCommands() {
  while (Serial.available() > 0) {
    ...
    if (ch == 't') {
      ...
      g_device.setLevel(static_cast<uint8_t>(nextLevel));
      applyLedState();
      persistState();
      Serial.print("level=");
      Serial.print(g_device.level());
...
void setup() {
  ...
  basic.modelIdentifier = "X54-STATIC-DIM";
  basic.swBuildId = "0.3.0";
  basic.powerSource = 0x01U;
  g_device.configureDimmableLight(kLocalEndpoint, kIeeeAddress, kLocalShort,
                                  kPanId, basic, 0x0000U);

  g_store.begin("zbhadim");
  restoreState();
  applyLedState();

  const bool ok = g_radio.begin(kChannel, 8);
  Serial.print("\r\nZigbeeHaDimmableLightStatic start\r\n");
  Serial.print("radio=");
  ...
  Serial.print("\r\n");
  Serial.print("serial commands: t=toggle U=brighter D=dimmer M=mid r=report s=status c=clear\r\n");
}
