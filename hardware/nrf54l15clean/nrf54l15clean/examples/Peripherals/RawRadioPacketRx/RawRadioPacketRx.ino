...
PowerManager gPower;
alignas(4) uint8_t gPacket[1U + kConfigMaxPayloadLength];
...
bool configureRadio() {
  ...
  uint32_t pcnf1 = 0U;
  pcnf1 |= (kConfigMaxPayloadLength << RADIO_PCNF1_MAXLEN_Pos) &
...
void printPacket() {
  const uint8_t payloadLen = min<uint8_t>(gPacket[0], kConfigMaxPayloadLength);
  if (payloadLen < 2U) {
    Serial.println(F("RX short packet"));
    return;
  }

  const uint16_t counter = static_cast<uint16_t>(gPacket[1]) |
                           (static_cast<uint16_t>(gPacket[2]) << 8U);

  Serial.print(F("RX counter="));
  Serial.print(counter);
  Serial.print(F(" payload=\""));
  for (uint8_t i = 2U; i < payloadLen; ++i) {
    const char ch = static_cast<char>(gPacket[1U + i]);
    if ((ch >= 32) && (ch <= 126)) {
      Serial.print(ch);
    } else {
      Serial.print('.');
