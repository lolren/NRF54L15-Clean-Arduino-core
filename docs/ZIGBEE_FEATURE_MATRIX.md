...
| NWK join state machine | Partial | `zigbee_commissioning` now owns shared end-device commissioning state, active-scan candidate selection, association wait handling, parent polling while waiting for Transport Key or Update Device, retry/timeout policy, MAC orphan notification plus coordinator realignment for retained-key parent recovery, NWK-secured rejoin request/response before the old reassociation fallback, explicit `rejoin_verify` and `leave_reset` phases, and retained-key demo rejoin state transitions for the joinable HA examples, including a masked-channel fallback scan for the retained PAN or EPID when the first secure-rejoin attempt misses. Plain leave now stays in reset instead of immediately falling back into automatic fresh steering, but the on-air behavior is still not full Zigbee 3.0 commissioning. |
...
| ZDO Management Leave request/response | Partial | Codec support now exists, and the joinable HA endpoints accept a leave request, send the ZDO status response, transition into retained-key secure rejoin when requested, and otherwise clear persisted joined-state without immediately starting fresh steering again. |
...
| BDB commissioning / network steering / rejoin | Partial | Core requirement for Home Assistant and Zigbee2MQTT interoperability. A shared commissioning state machine now exists in-tree for the joinable HA examples, including parent polling during trust-center wait states, retry/timeout handling, shared scheduling and retry throttling of End Device Timeout requests after join or secure rejoin, retained-network masked-channel fallback scanning, MAC orphan notification plus coordinator realignment, best-effort MAC ACK request/reply handling on the 802.15.4 path, NWK-secured rejoin request/response before the reassociation fallback, and fallback from exhausted secure-rejoin attempts back to fresh steering, but the current flow is still demo-network behavior rather than BDB-compliant Zigbee 3.0 commissioning. See `docs/ZIGBEE_3P0_PARITY_PLAN.md`. |
...
- Accept management leave requests on the joinable HA endpoint examples, holding a plain leave in explicit reset state instead of immediately auto-steering again or transitioning into retained-key secure rejoin when the leave request sets the rejoin flag.
