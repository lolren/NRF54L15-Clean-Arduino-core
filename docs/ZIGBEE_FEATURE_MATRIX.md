...
| APS acknowledgements / binding / group delivery | Partial | Clean binding-table handling, ZDO Bind/Unbind request/response support, demo-network APS group delivery, outbound HA report routing that now honors either extended or group bindings, unicast APS ACK generation/consumption, duplicate suppression, and small-slot bounded unicast retransmission now exist for the clean coordinator and joinable HA endpoints instead of a single pending-ACK demo slot. Broader APS reliability policy and real Zigbee multicast semantics remain open. |
| APS security / transport-key handling | Partial | The clean demo coordinator can now select a per-node preconfigured link key, including install-code-derived keys for the in-tree demo nodes, install the demo network key through APS-secured Transport Key, stage an alternate demo network key, refresh duplicate active or staged keys without resetting NWK replay state, reject stale or conflicting Transport Key sequence reuse, and follow retained-key demo rejoins with APS-secured `Update Device` plus APS-secured `Switch Key` rollout. The joinable HA examples now also require the expected trust-center source, commissioning lifecycle state, and inbound APS counter progression for `Transport Key`, `Update Device`, and `Switch Key` acceptance, they accept those APS-secured trust-center commands when the current parent relays them, and they surface rejected trust-center command handling through explicit commissioning failure state instead of only timing out later. Full APS security coverage, trust-center lifecycle behavior, and third-party interop are still missing. |
| ZDO Node Descriptor request/response | Foundation | Response builders, client-side request construction, and response parsing now exist. |
| ZDO Power Descriptor request/response | Foundation | Response builders, client-side request construction, and response parsing now exist. |
...
| ZDO Management Bind / LQI / Routing request handling | Partial | Joinable HA endpoints now answer `Mgmt_Bind_req` from the in-memory binding table, and they return explicit `Not Supported` responses for `Mgmt_Lqi_req` and `Mgmt_Rtg_req` instead of timing out. Full management/topology behavior is still missing. |
...
| ZCL write-attributes request/response | Partial | Shared HA server handling now supports `Write Attributes`, `Write Attributes Undivided`, and `Write Attributes No Response` for writable attributes currently modeled in-tree, most importantly `IdentifyTime`; undivided writes now validate atomically and refuse partial mutation when any record is read-only or otherwise invalid. |
| ZCL discover-attributes request/response | Partial | Client request build plus shared server response build/parse now exist for both standard and extended attribute discovery across the HA attribute sets currently modeled in-tree, including the standard `FeatureMap` and `ClusterRevision` global attributes on those clusters. Extended discovery currently reports the access flags this stack really supports today: read, write, and report. |
...
| ZCL configure-reporting request/response | Foundation | Client request build plus shared response parsing/building now exist. |
...
| Identify cluster handling | Partial | Identify Time reads plus `Identify`, `Identify Query`, `Trigger Effect`, direct `Write Attributes` / `Write Attributes Undivided` / `Write Attributes No Response` updates for `IdentifyTime`, and extended attribute discovery on the shared server path are implemented. The shared HA device runtime now maintains Identify Time countdown state and active effect selection so HA light and joinable sensor examples can render distinct blink/breathe/channel-change patterns without each sketch re-implementing the timer, and the coordinator join demo can now exercise identify/query/trigger-effect, identify-time-write, and identify extended-discovery flows against discovered HA nodes. |
...
| Real joined end device on an existing coordinator | Partial | Clean joinable On/Off Light, Dimmable Light, and Temperature Sensor examples now work against the in-tree clean coordinator demo, no longer hard-code that trust center into their security checks, can recover retained-key parent loss through orphan notification plus coordinator realignment and NWK-secured rejoin request/response before reassociation fallback, can negotiate End Device Timeout on the joined demo network, can accept APS-secured trust-center follow-up relayed by the current parent after join or secure rejoin, can retry secure rejoin by scanning for the retained network across configured channel masks, and expose preferred EPID plus local identity/install-code overrides as build-time policy knobs instead of requiring sketch edits. The joinable examples now also expose commissioning timing knobs for slower third-party coordinators, and the first-attempt settings are documented in `docs/ZIGBEE_HOME_ASSISTANT_BRINGUP.md`, but third-party coordinator interoperability is still missing. |
| Router role | Missing | Requires MAC+NWK routing, security, and persistence. |
| Coordinator role | Partial | Clean coordinator demo now performs beaconing, timed permit-join enforcement, association, address allocation, queued delivery over MAC data-request polling, unicast APS ACK handling with small-slot bounded retransmission, a ZHA-like early interview pass through Node Descriptor, Power Descriptor, Active EP, Simple Descriptor, and richer Basic-cluster reads, ZDO binding setup, reporting setup with readback verification, demo leave requests, orphan notification handling plus coordinator realignment for known retained-key nodes, NWK-secured rejoin response plus End Device Timeout response handling, retained-key demo rejoin handling via APS-secured Update Device, and a polled alternate-network-key rollout via APS-secured Transport Key plus Switch Key, along with demo On/Off and Level Control commands for joined HA lights and temperature sensors, including a shared light-control group. It now also tracks per-node trust-center lifecycle state and takes coordinator identity plus known-node install-code policy from build-time macros instead of fixed sketch constants. Trust-center/security/routing remain open. |
...
- Accept shared `Write Attributes`, `Write Attributes Undivided`, and `Write Attributes No Response` handling for modeled writable HA attributes such as `IdentifyTime`, while returning explicit read-only status for known read-only attributes and keeping undivided writes atomic.
...
- Answer ZDO management bind requests from the current binding table and fail management LQI/routing requests explicitly instead of silently timing out.
...
- Tune the shared commissioning engine for an external coordinator by policy instead of source edits, including active-scan, association-response, coordinator-realignment, and NWK-rejoin wait windows.
...
- Exchange unicast APS ACK frames between the clean coordinator and joinable HA endpoints for ZDO and HA application traffic, with small-slot bounded retransmission and duplicate suppression on the clean demos.
