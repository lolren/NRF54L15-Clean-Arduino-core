...
| BDB commissioning / network steering / rejoin | Partial | Core requirement for Home Assistant and Zigbee2MQTT interoperability. A shared commissioning state machine now exists in-tree for the joinable HA examples, including parent polling during trust-center wait states, retry/timeout handling, negotiated End Device Timeout requests after join or secure rejoin, retained-network masked-channel fallback scanning, MAC orphan notification plus coordinator realignment, best-effort MAC ACK request/reply handling on the 802.15.4 path, NWK-secured rejoin request/response before the reassociation fallback, and fallback from exhausted secure-rejoin attempts back to fresh steering, but the current flow is still demo-network behavior rather than BDB-compliant Zigbee 3.0 commissioning. See `docs/ZIGBEE_3P0_PARITY_PLAN.md`. |
