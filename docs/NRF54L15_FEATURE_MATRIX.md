...
| [ ] | Privacy / RPA helpers | HAL and Bluefruit APIs can generate RPAs, set a local RPA advertiser/scanner address, and resolve RPAs through the hardware AAR block. `BLE > Privacy > BleResolvablePrivateAddress` compiles as a user-facing example. | Automatic periodic RPA rotation, controller resolving-list policy, bonded reconnect identity handling, and phone/host privacy interop remain future work. |
