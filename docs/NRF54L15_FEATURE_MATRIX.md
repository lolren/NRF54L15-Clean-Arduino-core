...
| [x] | `SPIM` | `SPIM00`, `SPIM20`, `SPIM21`, `SPIM22`, `SPIM30` | Partial | Arduino `SPI` and HAL `Spim` exist; explicit `SPIM22` and `SPIM30` examples are present, but broader HS behavior still needs validation. |
...
| [x] | `TIMER` | `TIMER00`, `TIMER20`-`TIMER24` plus system timers | Mostly done | Arduino timing and timer-backed PWM use timers; a standalone `TIMER00` high-speed wrapper/example now exists, while broader validation across all instances remains open. |
| [x] | `TWIM` | `TWIM20`, `TWIM21`, `TWIM22`, `TWIM30` | Mostly done | `Wire`, `Wire1`, repeated-start, and HAL wrappers exist. |
| [x] | `TWIS` | `TWIS20`, `TWIS21`, `TWIS22`, `TWIS30` | Partial | I2C target callbacks exist and explicit `TWIS21`/`TWIS30` wrapper examples are present; broader target-mode validation remains open. |
...
| [x] | Dynamic 16-bit custom GATT | Implemented, including long reads, ATT Read Multiple Variable length/value tuple responses, contiguous queued prepare/execute writes for bounded custom characteristic values and writable `0x2901` User Description descriptors, bonded CCCD restore for matching peers, Bluefruit `setPermission()` read/write enforcement with encrypted-only versus MITM/authenticated checks, and read-only Bluefruit `setPresentationFormatDescriptor()` / `setReportRefDescriptor()` descriptors. | 128-bit dynamic service editing, descriptor host-app interop stress, and multi-service mutation remain open. |
| [x] | Bluefruit HID peripheral helpers | `BLEHidAdafruit` and `BLEHidGamepad` now expose HID Information, Report Map, Protocol Mode, Control Point, Report Reference descriptors, keyboard/mouse boot reports, notify real keyboard, mouse, consumer-control, and gamepad payloads, expose host Protocol Mode writes through `protocolMode()` / `setProtocolModeCallback()`, expose keyboard LED output reports through `keyboardLedState()` / `setKeyboardLedCallback()`, and let central HID sketches write Boot Keyboard Output LEDs through `BLEClientHidAdafruit::setKeyboardLedState()`. | Pairing behavior, OS host interoperability, boot-protocol switching against real hosts, and gamepad host behavior still need real-device validation. |
...
| [x] | LE Secure Connections pairing/bonding | Implemented for Just Works: two-board fresh pair, bond persistence, bonded reconnect encryption, encrypted notifications, and encrypted writes tested. Fixed-PIN, numeric-comparison, and mutual OOB paths are wired for two-core validation, authenticated bond state now restores MITM permissions on reconnect, and Bluefruit exposes authenticated-link queries for sketches. | Passkey/OOB host interop, one-way OOB/NFC/QR flows, optional ID/signing key distribution, and broader host interoperability remain future work. |
| [ ] | Privacy / RPA helpers | HAL and Bluefruit APIs can generate RPAs, set a local RPA advertiser/scanner address, resolve RPAs through the hardware AAR block, keep a small application-managed resolving list, inspect stored bond peer identity/IRK state, seed the resolving list from a bonded peer IRK, and opt into local RPA rotation before advertising, active scanning, or connection initiation. `BLE > Privacy > BleResolvablePrivateAddress` and `Bluefruit52Lib > Diagnostics > bond_identity_probe` compile as user-facing examples. | Automatic controller resolving-list policy, bonded reconnect identity handling, and phone/host privacy interop remain future work. |
...
| [x] | On/off light state model | Implemented with bounded endpoint/cluster descriptor lookup. | Bind to real Matter exchange path. |
...
| [ ] | Reboot/reconnect recovery | Partial | Thread dataset restore diagnostics now surface through the Matter on-network node and the `MatterOnNetworkRebootRecoveryProbe` example, but commissioned Matter session recovery is not validated. |
