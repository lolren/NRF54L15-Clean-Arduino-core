...
| [x] | Bluefruit HID peripheral helpers | `BLEHidAdafruit` and `BLEHidGamepad` now expose HID Information, Report Map, Protocol Mode, Control Point, Report Reference descriptors, keyboard/mouse boot reports, and notify real keyboard, mouse, consumer-control, and gamepad payloads. | Pairing behavior, OS host interoperability, boot-protocol switching, and gamepad host behavior still need real-device validation. |
