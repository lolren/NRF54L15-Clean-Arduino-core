...
| [x] | Bluefruit HID peripheral helpers | `BLEHidAdafruit` and `BLEHidGamepad` now expose HID Information, Report Map, Protocol Mode, Control Point, Report Reference descriptors, keyboard/mouse boot reports, notify real keyboard, mouse, consumer-control, and gamepad payloads, expose host Protocol Mode writes through `protocolMode()` / `setProtocolModeCallback()`, and expose keyboard LED output reports through `keyboardLedState()` / `setKeyboardLedCallback()`. | Pairing behavior, OS host interoperability, boot-protocol switching against real hosts, and gamepad host behavior still need real-device validation. |
