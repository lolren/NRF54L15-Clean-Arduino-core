...
| [x] | Dynamic 16-bit custom GATT | Implemented, including long reads, contiguous queued prepare/execute writes for bounded custom characteristic values and writable `0x2901` User Description descriptors, Bluefruit `setPermission()` read/write enforcement with encrypted-only versus MITM/authenticated checks, and read-only Bluefruit `setPresentationFormatDescriptor()` / `setReportRefDescriptor()` descriptors. | 128-bit dynamic service editing, descriptor host-app interop stress, and multi-service mutation remain open. |
