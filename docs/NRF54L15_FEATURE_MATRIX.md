...
| [x] | Dynamic 16-bit custom GATT | Implemented, including long reads, contiguous queued prepare/execute writes for bounded custom characteristic values, Bluefruit `setPermission()` read/write enforcement with encrypted-only versus MITM/authenticated checks, and read-only Bluefruit `setUserDescriptor()`, `setPresentationFormatDescriptor()`, and `setReportRefDescriptor()` descriptors. | 128-bit dynamic service editing, writable descriptor variants, and multi-service mutation remain open. |
