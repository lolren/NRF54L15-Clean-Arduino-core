...
| [ ] | Privacy / RPA helpers | HAL and Bluefruit APIs can generate RPAs, set a local RPA advertiser/scanner address, resolve RPAs through the hardware AAR block, keep a small application-managed resolving list, and opt into local RPA rotation before advertising, active scanning, or connection initiation. `BLE > Privacy > BleResolvablePrivateAddress` compiles as a user-facing example. | Automatic controller resolving-list policy, bonded reconnect identity handling, and phone/host privacy interop remain future work. |
