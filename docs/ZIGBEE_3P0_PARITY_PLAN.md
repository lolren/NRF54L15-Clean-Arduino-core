...
Status: partially landed. The joinable HA examples now use shared commissioning state, active scan, association, parent polling while waiting for Transport Key or Update Device, retry/timeout handling, shared scheduling of `Device Announce` plus End Device Timeout requests after join or secure rejoin, masked-channel fallback scanning for retained-network secure rejoin, MAC orphan-notification plus coordinator-realignment recovery, NWK-secured rejoin request/response before the reassociation fallback, retained-key rejoin transitions through `zigbee_commissioning`, and explicit `rejoin_verify` / `leave_reset` commissioning states so plain leave no longer falls straight back into automatic fresh steering. The behavior is still demo-network commissioning rather than BDB.
