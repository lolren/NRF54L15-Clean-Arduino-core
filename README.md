...
The Zigbee examples now include a clean demo-network secured NWK path with AES-CCM* MIC-32 protection, persisted frame counters, replay checks, and an APS-secured Transport Key install step protected by the ZigBeeAlliance09 link key between the joinable HA examples and `ZigbeeHaCoordinatorJoinDemo`. Zigbee 3.0 commissioning, full trust-center behavior, and third-party coordinator interoperability are still not implemented; see `docs/ZIGBEE_FEATURE_MATRIX.md`.
