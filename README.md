...
- BLE LE Secure Connections currently covers verified Just Works, fixed-PIN, and two-board numeric-comparison pairing on the clean BLE path. Mutual OOB pairing is wired for two-core testing, but still needs wider host/NFC/QR interoperability validation. See [BLE status](#ble--production-quality)
...
| | BLE: privacy/RPA helpers [¹](#ble-status) | ⚠️ |
...
- [ ] **HID host interoperability** — `BLEHidAdafruit` and `BLEHidGamepad` now expose HID Information, Report Map, Protocol Mode, Control Point, Report Reference descriptors, boot keyboard/mouse characteristics, and notify keyboard/mouse/consumer/gamepad report data. Host Protocol Mode writes are tracked, exposed through `protocolMode()` / `isBootProtocolMode()`, and can be logged with `setProtocolModeCallback()`. Host keyboard LED output reports are tracked through `keyboardLedState()` and `setKeyboardLedCallback()`, and central HID sketches can write Boot Keyboard Output LEDs with `BLEClientHidAdafruit::setKeyboardLedState()`. Pairing behavior, OS report parsing, and gamepad host behavior still need real-device validation.
...
- [ ] **Privacy / Resolvable Private Addresses** — sketches can now generate an RPA from an IRK, set it as the local random private address, resolve RPAs through the hardware AAR block, keep a small application-managed resolving list of peer IRKs, inspect stored bond peer identity/IRK details, seed the resolving list from a bonded peer IRK, and opt into automatic local RPA rotation for advertising/active-scan/connection-initiation paths. See `BLE > Privacy > BleResolvablePrivateAddress` and `Bluefruit52Lib > Diagnostics > bond_identity_probe`. Automatic controller resolving-list policy, bonded reconnect identity selection, and wider phone/desktop privacy interop are still future work.
...
| Limitation | Detail |
|---|---|
| **BLE LE Secure Connections** | Just Works, fixed-PIN, and two-board numeric comparison are verified on the clean BLE path. Mutual OOB is wired for two-core testing. Broader phone/host interoperability, passkey-entry validation against non-core peers, one-way OOB/NFC/QR flows, and optional identity/signing key distribution still need more work. |
...
values on both sides before encrypted traffic resumes.
...
BleOobPairCentral`; each board prints a `peer <r> <c>` line that must be pasted
